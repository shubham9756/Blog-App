...
const MyBlog = () => {
  ...
  useEffect(() => {
    const fetchMyBlogs = async () => {
      try {
        const res = await fetch(`http://localhost:1000/myBlog/${id}`, {
          credentials: "include",
        });
 ...
 const handleDeleteBlog = async (blogId) => {
  ...
  try {
    const res = await fetch(`http://localhost:1000/blog/delete/${blogId}`, {
      method: "DELETE",
      credentials: "include",
    });
        ...
        <div className="blog-grid">
          {blogs.map((blog) => (
            <div className="blog-card" key={blog._id}>
              <img
                src={`http://localhost:1000/upload/${blog.thumbnail}`}
                alt={blog.title}
