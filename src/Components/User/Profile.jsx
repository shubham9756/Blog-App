...
const Profile = () => {
  ...
  useEffect(() => {
    const callApi = async () => {
      const res = await fetch("http://localhost:1000/checkUser", {
              ...
              <div className="card-body">
                ...
                <div className="mb-3">
                  <div className="rounded-circle bg-secondary text-white d-inline-flex justify-content-center align-items-center"
                    ...
                    {user?.profileImage ? (
                      <img src={`http://localhost:1000/upload/${user?.profileImage}`} alt="Profile" className="rounded-circle" style={{ width: "100px", height: "100px", objectFit: "cover" }} />
                    ) : (
